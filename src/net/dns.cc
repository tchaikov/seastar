 ...
 *
 * - c-ares 1.7.0 - 1.33.x (ARES_VERSION >= 0x010700 && < 0x012200):
 *   * Uses ares_set_socket_functions (deprecated in 1.34.0+)
 *   * Uses ares_getsock for FD polling (less deprecated than ares_fds)
 *   * Uses ares_process for event processing
 *   * Uses ares_query_dnsrec for SRV records (if >= 1.28.0)
 *
 * - c-ares < 1.7.0:
 *   * Uses ares_set_socket_functions
 *   * Uses ares_fds for FD polling (most deprecated)
 *   * Uses ares_process for event processing
 *   * Uses ares_parse_srv_reply for SRV records
 */
...
private:
    ...
    // Socket state callback implementation for event-driven monitoring
    static void sock_state_callback(void* data, ares_socket_t socket_fd,
    ...
    , _timer(std::bind(&impl::poll_sockets, this))
{
    ...
    a_opts.sock_state_cb = sock_state_callback;
    a_opts.sock_state_cb_data = this;
    flags |= ARES_OPT_SOCK_STATE_CB;
    dns_log.info("Using ARES_OPT_SOCK_STATE_CB for event-driven socket monitoring (c-ares {}.{}.{})",
                 (ARES_VERSION >> 16) & 0xFF,
                 (ARES_VERSION >> 8) & 0xFF,
                 ARES_VERSION & 0xFF);
#endif
...
#if ARES_VERSION >= 0x011300  // c-ares 1.19.0+ supports ARES_OPT_SOCK_STATE_CB
void dns_resolver::impl::sock_state_callback(void* data, ares_socket_t socket_fd,
                                            int readable, int writable) {
    auto* impl = static_cast<dns_resolver::impl*>(data);
    impl->handle_socket_state_change(socket_fd, readable, writable);
}
...
#define USE_ARES_GETSOCK (ARES_VERSION >= 0x010700 && ARES_VERSION < 0x012200)  // ares_getsock only for intermediate versions
// Note: For c-ares >= 1.34.0, we use ares_process_fds without needing any FD polling
// For intermediate versions (1.7.0 - 1.33.x), we use ares_getsock (less deprecated than ares_fds)
// For very old versions (< 1.7.0), we fall back to ares_fds
...
#if USE_ARES_GETSOCK
        // Use ares_getsock for c-ares >= 1.7.0 (less deprecated than ares_fds)
...
#else
        // Fall back to deprecated ares_fds for very old c-ares versions
        fd_set readers, writers;
        FD_ZERO(&readers);
        FD_ZERO(&writers);
