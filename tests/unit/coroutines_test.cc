...
SEASTAR_TEST_CASE(test_async_generator_drained_buffered) {
    return test_async_generator_drained<circular_buffer>();
}
...
SEASTAR_TEST_CASE(test_async_generator_not_drained_buffered) {
    return test_async_generator_not_drained<circular_buffer>();
}
...
fiddle(coroutine::experimental::buffer_size_t size, int n, int* total) {
    for (int i = 0; i < n; i++) {
        if (i == 42) {
            throw std::invalid_argument("Eureka!");
        }
...
template<template<typename> class Container>
seastar::future<> test_async_generator_throws() {
    int total = 0;
    ...
    co_await count_to(100).then_wrapped([&total] (auto f) {
        BOOST_REQUIRE(f.failed());
...
SEASTAR_TEST_CASE(test_async_generator_throws_buffered) {
    return test_async_generator_throws<circular_buffer>();
}

SEASTAR_TEST_CASE(test_async_generator_throws_unbuffered) {
    return test_async_generator_throws<std::optional>();
}
